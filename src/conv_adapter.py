...
class ConvAdapterDesign1(nn.Module):
    ...
    def __init__(
        ...
        self.conv2 = nn.Conv2d(
            width, width, kernel_size=kernel_size, stride=stride,
            groups=groups, padding=padding, dilation=int(dilation),
        )
...
class ConvAdapter(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.conv1 = nn.Conv2d(
            inplanes, width, kernel_size=kernel_size, stride=stride,
            groups=groups, padding=padding, dilation=int(dilation),
        )
        self.act = act_layer()
        # point-wise conv
    ...
    def forward(self, x):
        ...
        out = self.conv2(out)
        out = out * self.se
        # add norm layer after conv2? (not in original repo, but might be useful)
        # out = self.norm2(out)
        return out
...
class LinearAdapter(nn.Module):
    """
    Design 2 v4, linear (fully-connected) version for non-conv layers.
    """
